...
- `multi-agents-orchestration.md`
  ...
  - 진입 경로와 명시된 역할에 따른 역할 판정, headless·Claude 세션 워커 호출, worktree 격리, 핸드오프 기준을 담습니다.
...
- 저장소·팀 고유 제약(용어, 컨벤션, 이 저장소만의 워크플로)은 모델 성능
  향상만을 이유로 자동 삭제하지 않습니다. 이런 규칙은 모델이 아니라 팀
  합의가 근거입니다.
- 반면 중복 지시, 특정 모델의 낮은 판단력을 보완하려던 세세한 규칙, 도구
  버전에 의존하는 운영 사실은 정기적으로 재검증하고, 근거가 사라졌으면
  축소하거나 뺍니다.
- 각 SSOT 문서에 개정이 필요하면 해당 절에 "언제 적용하는가"와 "언제
  갱신됐는가"만 남기고, 별도 "이 규칙은 영구적이다" 같은 문구는 넣지
  않습니다 — 문서마다 반복하면 문서끼리 어긋날 수 있으므로 이 원칙은 여기
  한 곳에서만 관리합니다.
