...
- [x] `jk-math`에 기존 EJU 수학 출제 규칙과 오래된 로컬 작업트리를 보존한 로밍·핸드오프 규칙을 확인·정정하고 원격 푸시한다.
- [x] `rabbit7-trader`는 `ai-hq`로 통합하고 별도 적용 대상에서 제외한다.
- [x] `SCS Mod modding`은 Git 저장소가 아니므로 제외한다.
- [x] 남은 적용 대상 프로젝트의 기존 규칙과 템플릿 차이를 각각 검토한다.
- [x] 남은 적용 대상 프로젝트에 필요한 공통 내용을 충돌 없이 추가 병합한다.
